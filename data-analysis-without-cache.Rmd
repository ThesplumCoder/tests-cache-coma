...
```{r}
ordered_queries_data <- queries_data[order(queries_data$COUNT_STAR, 
                                           queries_data$AVG_TIMER_WAIT, 
                                           decreasing = TRUE), ]
unique_queries_data <- ordered_queries_data[!duplicated(ordered_queries_data$DIGEST), ]
```

```{r}
plot(seq(to = 100), unique_queries_data$COUNT_STAR[1:100])
plot(seq(to = 100), unique_queries_data$AVG_TIMER_WAIT[1:100])

hist(ordered_queries_data$AVG_TIMER_WAIT, 
     breaks = 30)

boxplot(ordered_queries_data$AVG_TIMER_WAIT, 
        outline = FALSE, 
        horizontal = TRUE)
```
